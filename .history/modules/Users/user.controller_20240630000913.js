...
export async function signup(req, res) {
  try {
    ...
    const result = await get().collection('customers').insertOne(customer);
...
export async function signin(req, res) {
  try {
    ...
    const customer = await get().collection('customers').findOne({ email });
    
    if (!customer || !(await compare(password, customer.password))) {
      return res.status(400).send({ error: 'Invalid login credentials' });
    }
    
    const token = sign({ _id: customer._id }, 'secret', { expiresIn: '1h' });
