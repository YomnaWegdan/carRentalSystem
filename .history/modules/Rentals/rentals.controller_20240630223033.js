...
exports.createRental = async (req, res) => {
    try {
      ...
      const carToRent = await db.get().collection('cars').findOne({ _id: new ObjectId(car) });
      
      if (!carToRent || carToRent.rentalStatus === 'rented') {
        return res.status(400).send({ error: 'Car is not available for rent' });
      }
      
      const rental = { car: new ObjectId(car), customer: new ObjectId(customer), rentalDate, returnDate };
      const result = await db.get().collection('rentals').insertOne(rental);
      
      await db.get().collection('cars').updateOne({ _id: new ObjectId(car) }, { $set: { rentalStatus: 'rented' } });
