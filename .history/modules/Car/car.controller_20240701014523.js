...
const deleteCar = async (req , res)=>{
    ...
    const getCarsByModel = async (req, res) => {
        try {
          ...
          if (!model) {
            return res.status(400).send({ error: 'Model query parameter is required' });
          }
      
          const cars = await db.collection('cars').find({ model }).toArray();
          res.json({ message: 'success', cars });
        } catch (error) {
          console.error('Error fetching cars by model:', error); // Log the error for debugging
          res.status(500).send({ error: 'An error occurred while fetching cars by model' });
        }
      };
      ...
      const getRentedOrSpecificModelCars = async (req, res) => {
        try {
          ...
          console.error('Error fetching rented or specific model cars:', error); // Log the error for debugging
          res.status(500).send({ error: 'An error occurred while fetching rented or specific model cars' });
        }
      };
